...
public class ServerPacketHandler extends MSCPacketHandler {
    ...
    public void handleSettings(SettingsPacket pkt, EntityPlayer player) {
        SpawnSettings settings = MobSpawnControls.instance.getConfig()
            .getSettings();
        ...
        if (wasMasterEnabled != settings.isMasterEnabled()) // Resync
            MobSpawnControls.instance.getConfig()
                .sync();
    }

    @Override
    protected boolean hasPermission(EntityPlayer player) {
        return MobSpawnControls.instance.getConfig()
            .canPlayerEdit(player);
    }
    ...
    public void handleRequest(PacketType type, String dat, EntityPlayer player) {
        ...
        switch (type) {
            case SETTINGS:
                MobSpawnControls.DISPATCHER.sendTo(
                    MobSpawnControls.instance.getConfig()
                        .getSettings()
                        .createPacket(),
                    mpPlayer);
    ...
    private void handleEvaluatedBiomeRequest(String data, EntityPlayerMP player) {
        ...
        MSCPacket p = MobSpawnControls.instance.getConfig()
            .getModConfig(mod)
            .getSpawnMap()
            .getEvaluatedBiomePacket(mod, biome);
    ...
    private void handleEvaluatedGroupRequest(String data, EntityPlayerMP player) {
        ...
        MSCPacket p = MobSpawnControls.instance.getConfig()
            .getModConfig(mod)
            .getSpawnMap()
            .getEvaluatedGroupPacket(mod, group);
        MobSpawnControls.DISPATCHER.sendTo(p, player);
    }
    ...
    private void handleBiomeSettingRequest(String data, EntityPlayerMP player) {
        int colon = data.indexOf(':');
        String mod = data.substring(0, colon), biome = data.substring(colon + 1);
        ModConfig modConfig = MobSpawnControls.instance.getConfig()
            .getModConfig(mod);
        MSCPacket p = modConfig.getSpawnMap()
            .getBiomeSettingPacket(mod, biome, modConfig);
        MobSpawnControls.DISPATCHER.sendTo(p, player);
    }
    ...
    private void handleEntitySettingRequest(String data, EntityPlayerMP player) {
        ...
        ModConfig config = MobSpawnControls.instance.getConfig()
            .getModConfig(mod);
        Class<? extends EntityLiving> entityClass = config.getEntityClass(entity);
        EnumCreatureType type = config.getTypeMap()
            .get(entityClass);

        MSCPacket p = config.getSpawnMap()
            .getEntitySettingPacket(mod, entity, type, entityClass);
        MobSpawnControls.DISPATCHER.sendTo(p, player);
    }
    ...
    public void handleGroups(GroupsPacket packet, EntityPlayer player) {
        ModConfig config = MobSpawnControls.instance.getConfig()
            .getModConfig(packet.mod);
        for (String s : packet.biomeNames) config.processGroupCommand(s);

        SpawnMap map = config.getSpawnMap();
        for (String s : packet.groups) map.parseGroup(s);
    ...
    private void handleCreatureTypeRequest(String data, EntityPlayerMP player) {
        ...
        List<String> ent = MobSpawnControls.instance.getConfig()
            .getModConfig(mod)
            .getEntityNames(type);
    ...
    private void handleGroupsRequest(String mod, EntityPlayerMP player) {
        List<BiomeGroup> biomeGroups = MobSpawnControls.instance.getConfig()
            .getModConfig(mod)
            .getBiomeGroups();
        String[] groups = new String[biomeGroups.size()];
        for (int i = 0; i < groups.length; ++i) groups[i] = biomeGroups.get(i)
            .toString();
        String[] biomes = BiomeNameHelper.getAllBiomeNames()
    ...
    private void handleEntityListRequest(String mod, EntityPlayerMP player) {
        ...
        for (EnumCreatureType type : EnumCreatureType.values()) names[type.ordinal()] = config.getEntityNames(type)
            .toArray(new String[0]);
    ...
    public void handleEntitySetting(EntitySettingPacket packet, EntityPlayer player) {
        ...
        EnumCreatureType type = config.getTypeMap()
            .get(entityClass);
        config.getSpawnMap()
            .setEntitySettings(config, entityClass, type, packet.entries);
    ...
    public void handleBiomeSetting(BiomeSettingPacket packet, EntityPlayer player) {
        ...
        BiomeGroup group = (biome == null && !packet.biome.equalsIgnoreCase("master"))
            ? config.getBiomeGroup(packet.biome)
            : null;
        SpawnMap map = config.getSpawnMap();
        ...
        for (EnumCreatureType type : EnumCreatureType.values()) {
            ...
            if (group != null) map.setGroupSection(group, type, spawnList);
            else map.setBiomeSection(biome, type, spawnList);
        }
