...
public class ClientPacketHandler extends MSCPacketHandler {
    ...
    public void handleSettings(SettingsPacket pkt, EntityPlayer player) {
        if (mc.currentScreen instanceof MainMenu) mc.displayGuiScreen(new SettingsScreen(pkt, mc.currentScreen));
    }

    @Override
    public void handleAccessDenied(EntityPlayer player) {
        if (mc.currentScreen == null) mc.displayGuiScreen(new AccessDeniedScreen());
    }
    ...
    public void handleHandShake(EntityPlayer player) {
        if (mc.currentScreen == null) mc.displayGuiScreen(new MainMenu());
    }

    @Override
    public void handleModList(ModListPacket packet, EntityPlayer player) {
        if (mc.currentScreen instanceof MainMenu) mc.displayGuiScreen(new ModListScreen(packet, mc.currentScreen));
    }

    @Override
    public void handleDebug(DebugPacket packet, EntityPlayer player) {
        if (mc.currentScreen instanceof MainMenu) mc.displayGuiScreen(new DebugScreen(packet, mc.currentScreen));
    }
    ...
    public void handleGroups(GroupsPacket packet, EntityPlayer player) {
        if (mc.currentScreen instanceof ModListScreen) mc.displayGuiScreen(new GroupsMenu(packet, mc.currentScreen));
