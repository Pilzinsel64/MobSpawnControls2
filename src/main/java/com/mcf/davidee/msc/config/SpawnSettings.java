...
public class SpawnSettings {
            ...
            try {
                ...
                while (reader.ready()) {
                    ...
                    if (!line.startsWith("#")) {
                        ...
                        if (index == -1) warn("Expected character '='", num, line);
                        else setVariable(line.substring(0, index), line.substring(index + 1), num);
                    }
                    ++num;
                }
            } catch (IOException e) {
                MobSpawnControls.getLogger()
                    .throwing("SpawnSettings", "load", e);
            } finally {
                if (reader != null) {
                    try {
                        reader.close();
    ...
    private void warn(String message, int num, String line) {
        MobSpawnControls.getLogger()
            .warning(message + " in Settings.properties:" + num + " \"" + line + "\"");
    }

    // TODO Perhaps map values?
    private void setVariable(String key, String value, int line) {
        if (key.equalsIgnoreCase("masterenabled")) masterEnabled = "true".equalsIgnoreCase(value);
        else if (key.equalsIgnoreCase("readonly")) readOnly = "true".equalsIgnoreCase(value);
        else if (key.equalsIgnoreCase("creaturespawnfrequency"))
            creatureFreq = Utils.parseIntDMinMax(value, 400, 1, 400);
        else if (key.equalsIgnoreCase("caneditingame")) canEdit = "true".equalsIgnoreCase(value);
        else if (key.startsWith("Cap.")) {
            key = key.substring(4);
            EnumCreatureType type = MobHelper.typeOf(key);
            if (type == null) warn("Invalid Creature Type \"" + key + "\"", line, "Cap." + key + "=" + value);
            else {
                try {
        ...
        } else warn("Unknown variable setting \"" + key + "\"", line, key + "=" + value);
    }
