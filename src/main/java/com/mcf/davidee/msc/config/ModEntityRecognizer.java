...
public class ModEntityRecognizer {
    ...
    private static ListMultimap<ModContainer, EntityRegistration> getRegistrations() {
        return ObfuscationReflectionHelper
            .getPrivateValue(EntityRegistry.class, EntityRegistry.instance(), "entityRegistrations");
    }

    @SuppressWarnings("unchecked")
    public static List<Class<? extends EntityLiving>> getEntityClasses(ModContainer c) {
        if (c == null) return getVanillaEntityClasses();
        ...
        for (EntityRegistration r : regs) {
            if (isValidEntityClass(r.getEntityClass())) cls.add((Class<? extends EntityLiving>) r.getEntityClass());
        }
        return cls;
    }
    ...
    private static List<Class<? extends EntityLiving>> getVanillaEntityClasses() {
        ...
        for (Object o : EntityList.classToStringMapping.entrySet()) {
            ...
            if (isValidEntityClass(entry.getKey()) && EntityRegistry.instance()
                .lookupModSpawn(entry.getKey(), false) == null) cls.add((Class<? extends EntityLiving>) entry.getKey());
        }
        return cls;
    ...
    public static boolean hasEntities(ModContainer c) {
        for (EntityRegistration r : getRegistrations(c)) if (isValidEntityClass(r.getEntityClass())) return true;
        return false;
    }
