...
public class SpawnConfiguration {
    ...
    public SpawnConfiguration(File folder) {
        ...
        MobSpawnControls.getLogger()
            .info("Generating configs in directory \"" + folder.getAbsolutePath() + "\"");
        settings = new SpawnSettings(folder);
        configs = new ArrayList<ModConfig>();
        configs.add(new ModConfig(null, folder));
        for (ModContainer c : ModEntityFinder.getEntityMods()) configs.add(new ModConfig(c, folder));
    }
    ...
    public SpawnConfiguration(File folder, SpawnConfiguration _default) {
        ...
        configs = new ArrayList<ModConfig>();
        for (ModConfig c : _default.configs) configs.add(new ModConfig(c.container, folder, c));
    }
    ...
    public boolean canPlayerEdit(EntityPlayer p) {
        if (settings.canEdit() && p instanceof EntityPlayerMP) {
            ...
            if (mp.mcServer == null || mp.getCommandSenderName() == null) return false;
    ...
    public void save() {
        settings.save();
        for (ModConfig config : configs) config.save();
    }
    ...
    public void reEval() {
        for (ModConfig config : configs) config.getSpawnMap()
            .evaluate();
    }
    ...
    public void sync() {
        ...
        for (BiomeGenBase biome : BiomeNameHelper.getAllBiomes())
            for (EnumCreatureType type : EnumCreatureType.values()) BiomeReflector.reflectList(biome, type)
                .clear();

        // Step through the mods and each mod's entities
        for (ModConfig config : configs) config.getSpawnMap()
            .sync(settings.isMasterEnabled());
    }

    public void reset() {
        settings.reset();
        for (ModConfig config : configs) config.reset();
        sync();
    }
    ...
    public MSCPacket createModListPacket() {
        ...
        for (int i = 0; i < names.length; ++i) names[i] = configs.get(i).configName;
    ...
    public CreatureTypeMap getTypeMap(String mod) {
        for (ModConfig c : configs) if (c.configName.equalsIgnoreCase(mod)) return c.getTypeMap();
        return null;
    }

    public ModConfig getModConfig(String mod) throws IllegalArgumentException {
        for (ModConfig c : configs) if (c.configName.equalsIgnoreCase(mod)) return c;
