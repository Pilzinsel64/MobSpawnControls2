...
public class ModConfigParser {
    ...
    public BiomeGroup readBiomeGroup(String line) {
        ...
        if (index != -1) { // Group definition must contain a '='
            ...
            String[] parts = line.substring(index + 1)
                .split(",");
            for (int i = 0; i < parts.length; ++i) {
                ...
                if (op == null) // Must be valid op
                ...
                else if (op == BiomeOperator.ALL) if (i == 0) ops.add(new BiomeOperand());
                    ...
                    if (groupName.equalsIgnoreCase(name)) // Prevent self-references
                        ...
                        if (name.indexOf('.') != -1) { // Biome
                            ...
                            if (biome == null) log.add("Unknown biome \"" + name + "\"");
                            else ops.add(new BiomeOperand(op, biome));
                        } else { // Group Name
                            BiomeGroup group = config.getBiomeGroup(name);
                            if (group == null) log.add("Unknown group \"" + name + "\"");
                            else ops.add(new BiomeOperand(op, group));
                        }
                    }
                }
            }
    ...
    public void readBiomeSettings(String line) {
        ...
        if (index != -1) {
            ...
            if (indexOfType != -1) {
                ...
                BiomeGroup group = (biome == null && !biomeName.equalsIgnoreCase("master"))
                    ? config.getBiomeGroup(biomeName)
                    : null;
                ...
                if ((biome != null || biomeName.equalsIgnoreCase("master")) && creatureType != null)
                    ...
                    if (type == null) log.add("Invalid creature type \"" + type + "\"");
                    if (biome == null) log.add("Invalid biome/group \"" + biomeName + "\"");
                }
            } else log.add("Invalid configuration setting/command");
        } else log.add("Invalid configuration setting/command");
    }
    ...
    private void setGroupSettings(BiomeGroup group, EnumCreatureType creatureType, String[] entities) {
        ...
        config.getSpawnMap()
            .setGroupSection(group, creatureType, list);
    }
    ...
    private void setBiomeSettings(BiomeGenBase biome, EnumCreatureType creatureType, String[] entities) {
        ...
        config.getSpawnMap()
            .setBiomeSection(biome, creatureType, list);
    }
    ...
    private SpawnListEntry getEntry(String ent, EnumCreatureType creatureType) {
        if (!ent.isEmpty()) {
            ...
            if (startP != -1 && endP != -1) { // 3 args, with ( )
                ...
                String[] args = ent.substring(startP + 1, endP)
                    .split("-");
                if (args.length == 3) {
                    ...
                    if (entityClass != null) return new SpawnListEntry(entityClass, _weight, _min, _max);
                    else log.add("Unknown Entity \"" + ent + "\"");
                } else log.add("Invalid Entity setting \"" + ent + "\"");
            } else { // No args, just entity class name
                Class<? extends EntityLiving> entityClass = config.getEntityClass(ent, creatureType);
                if (entityClass != null) return new SpawnListEntry(entityClass, config.weight, config.min, config.max);
                else log.add("Unknown Entity \"" + ent + "\"");
            }
    ...
    public void setTypeSettings(String line) {
        ...
        if (index != -1) {
            ...
            String[] creatureNames = line.substring(index + 1)
                .split(",");
            if (!pre.equals("UNKNOWN")) { // Unknown denotes a 'warning' from our mod, noting that an entity
                ...
                if (type == null) log.add("Invalid Creature Type \"" + pre + "\"");
                    ...
                    for (String s : creatureNames) {
                        ...
                        if (c == null) log.add("Unknown Entity \"" + s + "\"");
                        else classes.add(c);
                    }
                    config.getTypeMap()
                        .setType(type, classes);
                }
            }
        } else log.add("Expected character ':'");
    }
