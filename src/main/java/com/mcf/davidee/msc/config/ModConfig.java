...
public class ModConfig {
    ...
    public ModConfig(ModContainer container, File folder, ModConfig c) {
        ...
        groups = new ArrayList<BiomeGroup>();
        for (BiomeGroup g : c.groups) groups.add(g.clone());
    ...
    public void save() {
        ...
        try {
            ...
            Utils.writeLine(writer, "# Mob Spawn Controls Configuration for \"" + configName + "\"");
            ...
            for (EnumCreatureType type : EnumCreatureType.values()) Utils.writeLine(writer, getTypeString(type));

            Utils.writeLine(writer, getTypeString(null));
            Utils.writeLine(writer, "#");
            Utils.writeLine(writer, "# Biome Groupings");
            // Write groups
            for (BiomeGroup group : groups) Utils.writeLine(writer, '&' + group.toString());
            Utils.writeLine(writer, "#");
    ...
    private void readGroup(String line) {
        ...
        if (group == null) warn(parser.log.get(0), '&' + line);
        else {
            for (String s : parser.log) warn(s, '&' + line);
    ...
    private void readVariable(String line) {
        ...
        if (index != -1) {
            ...
            if (var.equalsIgnoreCase("readonly")) readOnly = "true".equalsIgnoreCase(value);
            if (var.equalsIgnoreCase("itemweight")) weight = Utils.parseIntDMinMax(value, weight, 1, 100);
            if (var.equalsIgnoreCase("mincount")) min = Utils.parseIntDMinMax(value, min, 1, 10);
            if (var.equalsIgnoreCase("maxcount")) {
                max = Utils.parseIntDMinMax(value, max, 1, 15);
                if (min > max) max = min;
            }
        } else warn("Expected character '='", "?" + line);
    }
    ...
    private void warn(String message, String line) {
        MobSpawnControls.getLogger()
            .warning(message + " in " + fileName + ".cfg:" + lineNum + " \"" + line + "\"");
    }

    public BiomeGroup getBiomeGroup(String name) {
        for (BiomeGroup group : groups) if (group.getName()
            .equalsIgnoreCase(name)) return group;
        return null;
    }
    ...
    public List<String> getEntityNames(EnumCreatureType type) {
        ...
        for (Class<? extends EntityLiving> c : cls) strings.add(getEntityName(c));
    ...
    public void processGroupCommand(String command) {
        ...
        if (id.equalsIgnoreCase("del")) { // Delete group
            String name = args[1];
            for (Iterator<BiomeGroup> it = groups.iterator(); it.hasNext();) if (it.next()
                .getName()
