...
public class EditGroupScreen extends MSCScreen {
    ...
    protected void revalidateGui() {
        ...
        for (int i = 0; i < groupWidgets.size(); ++i) groupWidgets.get(i)
            .setPosition(width / 2 - 125, height / 4 - 8 + 14 * i);
        ...
        for (int i = 0; i < biomeWidgets.size(); ++i) biomeWidgets.get(i)
            .setPosition(width / 2 + 125, height / 4 - 8 + 14 * i);
    ...
    protected void createGui() {
        ...
        if (!groupDef.isEmpty()) {
            ...
            for (int i = 0; i < arr.length; ++i) def[i] = new FocusableSpecialLabel(getSimpleName(arr[i]), arr[i]);
    ...
    public void buttonClicked(Button button) {
        ...
        if (button == add) addGroupDefinition('+' + ((FocusableSpecialLabel) biomes.deleteFocused()).getSpecialText());
        if (button == subtract)
            addGroupDefinition('-' + ((FocusableSpecialLabel) biomes.deleteFocused()).getSpecialText());
        if (button == remove) addUnused(((FocusableSpecialLabel) group.deleteFocused()).getSpecialText());
        if (button == inspect) {
            ...
            if (lb.getSpecialText()
                .contains(".")) showBiomePopup(lb.getText(), lb.getSpecialText());
            else showGroupPopup(lb.getSpecialText());
    ...
    private void showGroupPopup(String group) {
        if (group.equals("*")) mc.displayGuiScreen(new PopupMessage(this, "OK", "", "", "* (All Biomes)"));
        else {
            if (group.startsWith("+") || group.startsWith("-")) group = group.substring(1);
            mc.displayGuiScreen(new PopupMessage(this, "OK", "", "", "Group \"" + group + "\""));
        }
    }

    private void addGroupDefinition(String defLine) {
        int i = group.getFocusableWidgets()
            .size();
        group.addWidgets(
            new FocusableSpecialLabel(width / 2 - 125, height / 4 - 8 + i * 14, getSimpleName(defLine), defLine));
    ...
    private void addUnused(String line) {
        if (!line.equals("*")) {
            int i = biomes.getFocusableWidgets()
                .size();
            String toUse = line.substring(1);
            biomes.addWidgets(
                new FocusableSpecialLabel(width / 2 + 125, height / 4 - 8 + 14 * i, getSimpleName(toUse), toUse));
        }
