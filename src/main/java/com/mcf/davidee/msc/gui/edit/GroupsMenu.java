...
public class GroupsMenu extends MSCScreen {
    ...
    protected void revalidateGui() {
        ...
        for (int i = 0; i < groups.size(); ++i) groups.get(i)
            .setPosition(width / 2, height / 4 - 5 + i * 14);
    ...
    protected void createGui() {
        ...
        save = new ButtonVanilla(50, 20, "Save", new ButtonHandler() {
            ...
            public void buttonClicked(Button button) {
                ...
                MobSpawnControls.DISPATCHER
                    .sendToServer(MSCPacket.getPacket(PacketType.GROUPS, packet.mod, arr, log.toArray(new String[0])));
                close();
            }
        });
    ...
    @Override
    public void buttonClicked(Button button) {
        if (button == add) mc.displayGuiScreen(new GroupPopup(this, groupMap.keySet()));
        if (button == remove) {
        ...
        if (button == rename) mc.displayGuiScreen(
            new GroupPopup(this, ((FocusableLabel) labelContainer.getFocusedWidget()).getText(), groupMap.keySet()));
        if (button == edit) {
            String group = ((FocusableLabel) labelContainer.getFocusedWidget()).getText();
    ...
    private List<String> getUnused(String group) {
        ...
        for (int i = 0; i < groups.indexOf(group); ++i) list.add(groups.get(i));
        // Add all biomes
        for (String s : packet.biomeNames) list.add(s);
        ...
        if (!defString.isEmpty()) {
            ...
            for (String s : def) if (s.charAt(0) != '*') list.remove(s.substring(1));
        }
    ...
    public void addGroup(String group) {
        ...
        groups.add(group);
        groupMap.put(group, "");
        int i = labelContainer.getFocusableWidgets()
            .size();
        ...
        for (Entry<String, String> entry : groupMap.entrySet()) {
            ...
            for (String s : ops) {
                if (!s.isEmpty()) { // For each string
                    ...
                    if (c != '*') {
                        String gb = s.substring(1);
                        if (gb.equalsIgnoreCase(group)) gb = newName;
                        if (hasPrev) sb.append(',');
                        sb.append(c + gb);
                        hasPrev = true;
                    } else {
