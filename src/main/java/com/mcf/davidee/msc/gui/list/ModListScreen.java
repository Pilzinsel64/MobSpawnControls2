...
public class ModListScreen extends MSCScreen {
    ...
    protected void revalidateGui() {
        ...
        for (int i = 0; i < labels.length; ++i) labels[i].setPosition(width / 2, height / 4 - 5 + i * 14);
    ...
    protected void createGui() {
        ...
        for (int i = 0; i < labels.length; ++i) labels[i] = new FocusableLabel(packet.mods[i]);
    ...
    public void buttonClicked(Button button) {
        ...
        if (button == types) mc.displayGuiScreen(new MobTypesMenu(mod, this));
        if (button == controls) mc.displayGuiScreen(new SpawnControlMenu(mod, this));
        if (button == groups)
