...
public class EditCreatureTypeScreen extends MSCScreen {
    ...
    public void drawBackground() {
        ...
        drawRect(
            labelContainer.left(),
            labelContainer.top(),
            labelContainer.right() - 15,
            labelContainer.bottom(),
            0x44444444);
        drawRect(
            creatureContainer.left(),
            creatureContainer.top(),
            creatureContainer.right(),
            creatureContainer.bottom(),
            0x44444444);
    }
    ...
    protected void revalidateGui() {
        ...
        for (int i = 0; i < entities.size(); ++i) entities.get(i)
            .setPosition(width / 2 - 30, height / 4 - 23 + i * 14);

        List<FocusableWidget> types = creatureContainer.getFocusableWidgets();
        for (int i = 0; i < types.size(); ++i) types.get(i)
            .setPosition(width / 2 + 120, height / 4 + 27 + i * 14);
    ...
    protected void createGui() {
        ...
        save = new ButtonVanilla(50, 20, "Save", new ButtonHandler() {
            ...
            public void buttonClicked(Button button) {
                ...
                MobSpawnControls.DISPATCHER.sendToServer(
                    MSCPacket.getPacket(
                        PacketType.CREATURE_TYPE,
                        packet.mod,
                        packet.creatureType,
                        modifications.toArray(new String[0])));
                close();
            }
        });
        ...
        for (int i = 0; i < labels.length; ++i) labels[i] = new FocusableLabel(packet.mobs[i]);

        scrollbar = new ScrollbarVanilla(15);
        ...
        for (int i = 0; i < creatureLabels.length; ++i) creatureLabels[i] = new FocusableLabel(creatureNames[i]);
