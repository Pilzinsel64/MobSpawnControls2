...
public class BiomeListScreen extends MSCScreen {
    ...
    protected void revalidateGui() {
        ...
        for (int i = 0; i < groups.size(); ++i) groups.get(i)
            .setPosition(width / 2, height / 4 - 8 + i * 14);
    ...
    protected void createGui() {
        ...
        for (int i = 0; i < labels.length; ++i) labels[i] = new FocusableLabel(packet.biomes[i]);
            ...
            if (packet.evalRequest) {
                if (selected.contains(".")) // Biome
                    MobSpawnControls.DISPATCHER.sendToServer(
                        MSCPacket.getRequestPacket(PacketType.EVALUATED_BIOME, packet.mod + ':' + selected));
                else // Group
                    MobSpawnControls.DISPATCHER.sendToServer(
                        MSCPacket.getRequestPacket(PacketType.EVALUATED_GROUP, packet.mod + ':' + selected));
            } else MobSpawnControls.DISPATCHER
                .sendToServer(MSCPacket.getRequestPacket(PacketType.BIOME_SETTING, packet.mod + ':' + selected));
        }
    }
