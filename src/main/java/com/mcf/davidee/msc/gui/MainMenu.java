...
public class MainMenu extends MSCScreen {
    ...
    public void buttonClicked(Button button) {
        if (button == mods) MobSpawnControls.DISPATCHER.sendToServer(MSCPacket.getRequestPacket(PacketType.MOD_LIST));
        if (button == settings)
            MobSpawnControls.DISPATCHER.sendToServer(MSCPacket.getRequestPacket(PacketType.SETTINGS));
        if (button == debug) MobSpawnControls.DISPATCHER.sendToServer(MSCPacket.getRequestPacket(PacketType.DEBUG));
