...
public class EditBiomeScreen extends MSCScreen {
    ...
    protected void createGui() {
        ...
        masterContainer
            .addWidgets(title, subTitle, enabled, weight, min, max, monster, creature, ambient, water, save, close);
        ...
        if (labelContainer.hasFocusedWidget()) updateGuiSettings();
    ...
    public void buttonClicked(Button button) {
        if (button == save) {
            ...
            String type = button.getText()
                .replace("Water", "WaterCreature");
            int index = MobHelper.typeOf(type)
                .ordinal();
    ...
    private void save() {
        ...
        for (int index = 0; index < 4; ++index) {
            ...
            for (EntityEntry entry : entries[index]) if (entry.weight > 0) {
        ...
        MobSpawnControls.DISPATCHER
            .sendToServer(MSCPacket.getPacket(PacketType.BIOME_SETTING, packet.mod, packet.biome, newEntries, empty));
    }
