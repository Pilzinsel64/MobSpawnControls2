...
public class BiomeSetting {
    ...
    public BiomeSetting(BiomeGenBase biome, List<Class<? extends EntityLiving>> validClasses) {
        ...
        List<SpawnListEntry> m = SpawnList
            .copySpawnListEntries(BiomeReflector.reflectList(biome, EnumCreatureType.monster));
        List<SpawnListEntry> c = SpawnList
            .copySpawnListEntries(BiomeReflector.reflectList(biome, EnumCreatureType.creature));
        List<SpawnListEntry> w = SpawnList
            .copySpawnListEntries(BiomeReflector.reflectList(biome, EnumCreatureType.waterCreature));
        List<SpawnListEntry> a = SpawnList
            .copySpawnListEntries(BiomeReflector.reflectList(biome, EnumCreatureType.ambient));
    ...
    public void sync() {
        for (EnumCreatureType type : EnumCreatureType.values()) BiomeReflector.reflectList(biome, type)
            .addAll(active.getSpawnList(type));
    }
    ...
    private void mergeLists(List<SpawnListEntry> myEntries, List<SpawnListEntry> theirEntries) {
        for (SpawnListEntry entry : theirEntries) {
            ...
            if (!found) myEntries.add(SpawnList.copyEntry(entry));
        }
    }
    ...
    private void removeInvalidEntries(List<SpawnListEntry> entries, List<Class<? extends EntityLiving>> validClasses) {
        for (Iterator<SpawnListEntry> it = entries.iterator(); it.hasNext();)
            if (!validClasses.contains(it.next().entityClass)) it.remove();
    }
