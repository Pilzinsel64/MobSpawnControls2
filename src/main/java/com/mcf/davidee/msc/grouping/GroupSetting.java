...
public class GroupSetting {
    ...
    public void mergeInto(Map<BiomeGenBase, BiomeSetting> map) {
        for (BiomeGenBase biome : group.evaluate()) {
            ...
            if (s != null) s.merge(active);
            else {
                MobSpawnControls.getLogger()
                    .severe("Unknown biome (id=" + biome.biomeID + ") " + BiomeNameHelper.getBiomeName(biome));
                MobSpawnControls.getLogger()
                    .severe("Most likely there is a biome ID conflict");
                BiomeGenBase b = getBiome(map.keySet(), biome.biomeID);
                if (b != null) MobSpawnControls.getLogger()
                    .severe("Conflict biome (id=" + b.biomeID + ") " + BiomeNameHelper.getBiomeName(b));
                throw new RuntimeException(
                    "MSC: Unknown biome (id=" + biome.biomeID + ") " + BiomeNameHelper.getBiomeName(biome));
            }
        }
    }

    private static BiomeGenBase getBiome(Collection<BiomeGenBase> biomes, int id) {
        for (BiomeGenBase biome : biomes) if (biome.biomeID == id) return biome;
        return null;
    }
    ...
    public void write(BufferedWriter writer, String textToIgnore) throws IOException {
        for (EnumCreatureType type : EnumCreatureType.values()) Utils.writeLine(
            writer,
            group.getName() + "~" + type + ":" + SpawnList.entriesToString(active.getSpawnList(type), textToIgnore));
