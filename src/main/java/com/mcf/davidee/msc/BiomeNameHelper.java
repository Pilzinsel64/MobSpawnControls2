...
public class BiomeNameHelper {
    ...
    public static final List<Integer> VANILLA_MUT = Arrays
        .asList(129, 130, 131, 132, 133, 134, 140, 149, 151, 155, 156, 157, 158, 160, 162, 163, 164, 165, 166, 167);
    ...
    public static String getBiomeName(BiomeGenBase biome) {
        String name = ((isVanilla(biome)) ? "Vanilla"
            : biome.getClass()
                .getName())
            + '.'
            + biome.biomeName;
        for (Entry<String, String> entry : nameSimplificationMap.entrySet())
            if (name.startsWith(entry.getKey())) return entry.getValue() + '.' + biome.biomeName;
        return name;
    }
