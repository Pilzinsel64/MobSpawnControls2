...
public class MobHelper {
    ...
    public static void populateDefaultMap() {
        for (Class c : (Set<Class>) EntityList.classToStringMapping.keySet()) {
            ...
            if (ModEntityRecognizer.isValidEntityClass(c)) {
                boolean found = false;
                outer: for (BiomeGenBase biome : BiomeGenBase.getBiomeGenArray()) {
                    if (biome != null) {
                ...
                if (!found) defaultMap.set(c, null);
    ...
    public static EnumCreatureType getDefaultMobType(Class<? extends EntityLiving> cls) {
        ...
        if (type == null) {
            if (EntityAmbientCreature.class.isAssignableFrom(cls)) return EnumCreatureType.ambient;
            if (EntityWaterMob.class.isAssignableFrom(cls)) return EnumCreatureType.waterCreature;
            if (IMob.class.isAssignableFrom(cls)) return EnumCreatureType.monster;
            if (EntityCreature.class.isAssignableFrom(cls)) return EnumCreatureType.creature;
        }
        return type;
    }

    public static EnumCreatureType typeOf(String s) {
        for (EnumCreatureType t : EnumCreatureType.values()) if (t.toString()
            .equalsIgnoreCase(s)) return t;
        return null;
    }
}
