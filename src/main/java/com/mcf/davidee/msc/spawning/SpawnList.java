...
public class SpawnList {
    ...
    private static SpawnListEntry getEntityEntry(Class entityClass, List<SpawnListEntry> list) {
        for (SpawnListEntry e : list) if (e.entityClass == entityClass) return e;
        return null;
    }
    ...
    private void removeEntityEntry(Class entityClass, List<SpawnListEntry> list) {
        for (Iterator<SpawnListEntry> it = list.iterator(); it.hasNext();) if (it.next().entityClass == entityClass) {
            it.remove();
            break;
        }
    }
    ...
    public static List<SpawnListEntry> copySpawnListEntries(List<SpawnListEntry> list) {
        ...
        for (SpawnListEntry e : list) newList.add(copyEntry(e));
        return newList;
    }
    ...
    public static String entryToString(SpawnListEntry e) {
        return EntityList.classToStringMapping
            .get(e.entityClass) + "(" + e.itemWeight + "-" + e.minGroupCount + "-" + e.maxGroupCount + ")";
    }
    ...
    public static List<Class<? extends EntityLiving>> getDisabledEntities(List<SpawnListEntry> spawning,
        List<Class<? extends EntityLiving>> list) {
